...
def build_parser():
    ...
    parser.add_argument(
        "--check-done", action="store_true", help="flag to only check if the analysis task is " + "done"
    )
    parser.add_argument(
        "-t",
        "--analysis-task",
...
def _get_input_path(prompt):
    while True:
        ...
        if not pathString.startswith("s3://") and not os.path.exists(os.path.expanduser(pathString)):
            print("Directory %s does not exist. Please enter a valid path." % pathString)
        else:
            return pathString
...
def generate_analysis_tasks_and_snakefile(dataSet: dataset.MERFISHDataSet, parametersFile: TextIO) -> str:
    ...
    snakeGenerator = snakewriter.SnakefileGenerator(analysisParameters, dataSet, sys.executable)
