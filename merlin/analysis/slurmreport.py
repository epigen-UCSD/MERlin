...
class SlurmReport(analysistask.AnalysisTask):
    ...
    def _generate_slurm_report(self, task: analysistask.AnalysisTask):
        ...
        slurmJobDF = pandas.read_csv(io.StringIO(queryResult.stdout.decode("utf-8")), sep="|")
    ...
    def _clean_slurm_dataframe(inputDataFrame):
        ...
        outputDF = outputDF.assign(
            Elapsed=pandas.to_timedelta(outputDF["Elapsed"].apply(reformat_timedelta), unit="s")
        )
        outputDF = outputDF.assign(
            Timelimit=pandas.to_timedelta(outputDF["Timelimit"].apply(reformat_timedelta), unit="s")
        )
        outputDF = outputDF.assign(
            Queued=pandas.to_timedelta(
                pandas.to_datetime(outputDF["Start"]) - pandas.to_datetime(outputDF["Submit"]), unit="s"
            )
        )
    ...
    def _plot_slurm_report(self, slurmDF, analysisName):
        ...
        plt.subplot(1, 4, 1)
        plt.boxplot([slurmDF["MaxRSS"].str[:-1].astype(float), slurmDF["ReqMem"].str[:-2].astype(int)], widths=0.5)
    ...
    def _plot_slurm_summary(self, reportDict):
        ...
        setBoxColors(bp, "b")
        plt.xticks(np.arange(len(reportDict)), list(reportDict.keys()), rotation="vertical")
        plt.yscale("log")
        (hB,) = plt.plot([1, 1], "b-")
        (hR,) = plt.plot([1, 1], "r-")
        ...
        plt.vlines(np.arange(0.5, len(reportDict)), ymin=plt.ylim()[0], ymax=plt.ylim()[1], linestyles="dashed")
        plt.tight_layout(pad=1)
    ...
    def _run_analysis(self):
                ...
                if currentTask.is_complete():
                    ...
                    try:
                        requests.post(
                            ...
                            files={
                                "file": (
                                    ".".join([t, self.dataSet.dataSetName, str(reportTime)]) + ".csv",
                                    dfStream.getvalue(),
                                )
                            },
                            timeout=10,
        ...
        datasetMeta = {
            ...
            "barcode_length": self.dataSet.get_codebook(self.parameters["codebook_index"]).get_bit_count(),
            "barcode_count": self.dataSet.get_codebook(self.parameters["codebook_index"]).get_barcode_count(),
            "fov_count": len(self.dataSet.get_fovs()),
            "z_count": len(self.dataSet.get_z_positions()),
            "sequential_count": len(self.dataSet.get_data_organization().get_sequential_rounds()),
        ...
        try:
            requests.post(
                "http://merlin.georgeemanuel.com/post",
                files={
                    "file": (".".join([self.dataSet.dataSetName, str(reportTime)]) + ".json", json.dumps(datasetMeta))
                },
                timeout=10,
            )
        except requests.exceptions.RequestException:
