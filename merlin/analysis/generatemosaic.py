...
class GenerateMosaic(analysistask.AnalysisTask):
    ...
    def run_analysis(self):
        micronExtents = self.global_align_task.get_global_extent()
    ...
    def _prepare_mosaic_slice(self, zIndex, dataChannel, micronExtents, alignTask, maximumProjection):
        ...
        for f in self.dataSet.get_fovs():
            if maximumProjection:
                inputImage = np.max(
                    [
                        self.warp_task.get_aligned_image(f, dataChannel, z, chromaticCorrector)
                        for z in range(len(self.dataSet.get_z_positions()))
                    ],
                    axis=0,
                )
            else:
                inputImage = self.warp_task.get_aligned_image(f, dataChannel, zIndex, chromaticCorrector)
