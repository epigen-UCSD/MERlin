...
class GenerateMosaic(analysistask.AnalysisTask):
    ...
    def _micron_to_mosaic_pixel(self, micronCoordinates, micronExtents) -> np.ndarray:
        ...
        global coordinates.
        """
        return np.matmul(self._micron_to_mosaic_transform(micronExtents), np.append(micronCoordinates, 1)).astype(
            np.int32
        )[:2]
    ...
    def _transform_image_to_mosaic(
        ...
        transform = np.matmul(self._micron_to_mosaic_transform(micronExtents), alignTask.fov_to_global_transform(fov))
        return cv2.warpAffine(inputImage, transform[:2, :], mosaicDimensions)

    def _run_analysis(self):
        alignTask = self.dataSet.load_analysis_task(self.parameters["global_align_task"])
        micronExtents = alignTask.get_global_extent()
        self.dataSet.save_numpy_txt_analysis_result(
            self._micron_to_mosaic_transform(micronExtents), "micron_to_mosaic_pixel_transform", self
        )
        ...
        if "data_channels" in self.parameters:
            if isinstance(self.parameters["data_channels"], str):
                dataChannels = [dataOrganization.get_data_channel_index(self.parameters["data_channels"])]
            elif isinstance(self.parameters["data_channels"], int):
                dataChannels = [self.parameters["data_channels"]]
            else:
                dataChannels = [
                    dataOrganization.get_data_channel_index(x) if isinstance(x, str) else x
        ...
        if not self.parameters["separate_files"]:
            imageDescription = self.dataSet.analysis_tiff_description(len(zIndexes), len(dataChannels))
            with self.dataSet.writer_for_analysis_images(self, "mosaic") as outputTif:
                for d in dataChannels:
                    for z in zIndexes:
                        mosaic = self._prepare_mosaic_slice(z, d, micronExtents, alignTask, maximumProjection)
                        outputTif.save(mosaic, photometric="MINISBLACK", metadata=imageDescription)
        else:
            imageDescription = self.dataSet.analysis_tiff_description(1, 1)
            for d in dataChannels:
                for z in zIndexes:
                    with self.dataSet.writer_for_analysis_images(
                        self, "mosaic_%s_%i" % (dataOrganization.get_data_channel_name(d), z)
                    ) as outputTif:
                        mosaic = self._prepare_mosaic_slice(z, d, micronExtents, alignTask, maximumProjection)
                        outputTif.save(mosaic, photometric="MINISBLACK", metadata=imageDescription)

    def _prepare_mosaic_slice(self, zIndex, dataChannel, micronExtents, alignTask, maximumProjection):
        ...
        mosaicDimensions = tuple(self._micron_to_mosaic_pixel(micronExtents[-2:], micronExtents))
        ...
        for f in self.dataSet.get_fovs():
            if maximumProjection:
                inputImage = np.max(
                    [
                        warpTask.get_aligned_image(f, dataChannel, z, chromaticCorrector)
                        for z in range(len(self.dataSet.get_z_positions()))
                    ],
                    axis=0,
                )
            else:
                inputImage = warpTask.get_aligned_image(f, dataChannel, zIndex, chromaticCorrector)
            ...
            cv2.add(mosaic, transformedImage, dst=mosaic, mask=np.array(transformedImage > 0).astype(np.uint8))
