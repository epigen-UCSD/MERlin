...
class MERFISHDataFactory(object):
    ...
    def simulate_image(self, spotPositions: np.ndarray = None, addNoise: bool = False) -> np.ndarray:
        ...
        for p in spotPositions:
            upsampledImage[
                int(np.floor(p[0] * self.upsampleFactor)), int(np.floor(p[1] * self.upsampleFactor))
            ] += 1000

        return self._downsample_image_stack([upsampledImage], addNoise=addNoise)[0]

    def simulate_dataset(self, datasetName, abundanceScale=1, fluorophoreCount=5, fovCount=10):
        ...
        barcodeAbundances = abundanceScale * np.array([10 ** np.random.uniform(3) for i in range(barcodeNumber)])
        barcodeAbundances[:10] = 0

        for i in range(fovCount):
            merfishImages, rnaPositions = self._simulate_single_fov(codebook, barcodeAbundances, fluorophoreCount)
            fiducialImage = self._simulate_fiducial_image()
            tifffile.imsave(os.sep.join([dataDir, "full_stack_" + str(i) + ".tiff"]), merfishImages.astype(np.uint16))

            imageCount = np.max([x[0] for x in self.bitOrganization]) + 1
            for j in range(imageCount):
                fileName = "Conventional_750_650_561_488_405_" + str(i) + "_" + str(j) + ".tiff"
                filePath = os.sep.join([dataDir, fileName])

                imageData = np.zeros(shape=(5, *self.imageSize), dtype=np.uint16)
                firstBitIndex = [i for i, x in enumerate(self.bitOrganization) if x[0] == j and x[1] == 0][0]
                secondBitIndex = [i for i, x in enumerate(self.bitOrganization) if x[0] == j and x[1] == 1][0]
            ...
            np.save(os.sep.join([dataDir, "true_positions_" + str(i) + ".npy"]), rnaPositions)
    ...
    def _simulate_single_fov(self, codebook, barcodeAbundances, fluorophoreCount):
        ...
        for b in range(barcodeCount):
            self._add_spots_for_barcode(codebook.get_barcode(b), rnaPositions[b], fluorophoreCount, upsampledStack)

        imageStack = self._downsample_image_stack(upsampledStack)

        return imageStack, rnaPositions

    def _add_spots_for_barcode(self, barcode, positions, fluorophoreCount, upsampledStack):
        upsampledImage = np.zeros(self.upsampleFactor * self.imageSize)
        for p in positions:
            upsampledImage[int(np.floor(p[0] * self.upsampleFactor)), int(np.floor(p[1] * self.upsampleFactor))] += 1
    ...
    def _downsample_image_stack(self, upsampledStack, addNoise=True):
        ...
        for i in range(len(imageStack)):
            blurredImage = cv2.GaussianBlur(
                upsampledStack[i].astype(float), ksize=(51, 51), sigmaX=self.upsampleFactor * self.psfSigma
            )
            downsampledImage = np.array(
                PIL.Image.fromarray(
                    convolve2d(blurredImage, np.ones((self.upsampleFactor, self.upsampleFactor)))
