...
def read_blist(bFile: BinaryIO) -> pandas.DataFrame:
    ...
    bytesPerEntry = int(np.sum([struct.calcsize(typeNames[x["type"]]) * np.prod(x["size"]) for x in entryFormat]))
    return pandas.DataFrame([_parse_entry_bytes(bFile.read(bytesPerEntry), entryFormat) for i in range(entryCount)])
...
def _read_binary_header(bFile: BinaryIO) -> Tuple[int, int, List[Dict]]:
    ...
    entryList = [
        {"name": x, "size": np.array(y.split("  ")).astype(int), "type": z} for x, y, z in _chunker(layout, 3)
    ]
...
def _parse_entry_bytes(byteList, entryFormat: List[Dict]):
    ...
    for currentEntry in entryFormat:
        ...
        items = np.array(
            [
                struct.unpack(itemType, byteList[byteIndex + i * itemSize : byteIndex + (i + 1) * itemSize])[0]
