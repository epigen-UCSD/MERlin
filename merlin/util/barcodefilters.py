...
def remove_zplane_duplicates_all_barcodeids(
    ...
    if len(barcodes) == 0:
        ...
        for bcGroup, bcData in barcodeGroups:
            bcToKeep.append(remove_zplane_duplicates_single_barcodeid(bcData, zPlanes, maxDist, allZPos))
...
def remove_zplane_duplicates_single_barcodeid(
    ...
    for z in range(0, len(zPos)):
        zToCompare = [
            pos for pos, otherZ in enumerate(zPos) if (pos >= z - zPlanes) & (pos <= z + zPlanes) & ~(pos == z)
        ]
    ...
    def choose_brighter_barcode(barcodes, indexes):
        sortedBC = barcodes.loc[indexes, :].sort_values(by="mean_intensity", ascending=False)
        return sortedBC.index.values.tolist()[0]

    keptBarcodes = barcodes.loc[
        sorted([x[0] if len(x) == 1 else choose_brighter_barcode(barcodes, x) for x in connectedComponents]), :
    ]
    return keptBarcodes
